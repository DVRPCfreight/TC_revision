...
var streetMap, imageMap, transMap, dvrpc, counts, getColor;
...
function getExtent(point) {
    var delta = 0.025;
    return new esri.geometry.Extent(point.x - delta, point.y - delta, point.x + delta, point.y + delta, new esri.SpatialReference({
        wkid: 102100
    }));
}
...
function getMCDExtent(lat, lng) {
    var delta = 900.025;
    return new esri.geometry.Extent(lat - delta, lng - delta, lat + delta, lng + delta, new esri.SpatialReference({
        wkid: 102100
    }));
}
...
$(document).ready(function() {
    ...
    var headerArr = [];
    $(colModel).each(function(i, val) {
        headerArr.push(val.label);
    });
...
function init() {
    ...
    dvrpc = new esri.layers.ArcGISDynamicMapServiceLayer(
        "http://arcgis.dvrpc.org/arcgis/rest/services/Boundaries/DVRPC_Boundaries/MapServer/"
    );
    map.addLayer(dvrpc);
    dvrpc.setVisibleLayers([1, 2]);
...
function createToolbar(map) {
    ...
    var boxSelect = new dijit.form.Button({
        ...
        onClick: function() {
            $("#selectButton").addClass("dijitButtonHover");
